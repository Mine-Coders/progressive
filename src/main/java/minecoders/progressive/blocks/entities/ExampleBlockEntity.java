...
public class ExampleBlockEntity extends StorageBlockEntity implements SidedInventory, SidedStorageBlockEntity, NamedScreenHandlerFactory {
    ...
    public static void tick(World world, BlockPos pos, BlockState state, ExampleBlockEntity blockEntity) {
        ...
        if (inputA.isEmpty() || !blockEntity.getStack(2).isEmpty()) {
            if (blockEntity.time > 0)
                --blockEntity.time;

            return;
        } else if (blockEntity.time < blockEntity.timeTotal) {
            ++blockEntity.time;
            return;
        }
    ...
    protected void readData(ReadView input) {
        ...
        this.time = input.getInt("time", 0);
        this.timeTotal = input.getInt("timeTotal", 20);
    }
    ...
    protected void writeData(WriteView output) {
        ...
        output.putInt("timeTotal", this.timeTotal);
    }
